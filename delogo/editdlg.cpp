...
static int TargetIDs[] = {
    ...
    ID_STATIC_X, ID_STATIC_Y, ID_STATIC_START, ID_STATIC_END, ID_STATIC_FIN, ID_STATIC_FOUT, ID_EDIT_AUTO_FADE, ID_EDIT_AUTO_NR,
};
...
void on_wm_initdialog(HWND hdlg)
{
    ...
    SendDlgItemMessage(hdlg, ID_EDIT_AUTO_FADE, BM_SETCHECK, !!lp->auto_fade, 0);
    SendDlgItemMessage(hdlg, ID_EDIT_AUTO_NR,   BM_SETCHECK, !!lp->auto_nr,   0);
...
static void on_wm_sizing(HWND hdlg, RECT *rect) {
    ...
    rect->right  = std::max(rect->right, rect->left + defaultWindow.w);
...
BOOL on_IDOK(HWND hdlg)
{
    ...
    newdata->st = (short)std::min((int)GetDlgItemInt(hdlg, ID_EDIT_START, NULL,  TRUE), track_e[6]);
    newdata->ed = (short)std::min((int)GetDlgItemInt(hdlg, ID_EDIT_END,   NULL,  TRUE), track_e[9]);
    newdata->fi = (short)std::min((int)GetDlgItemInt(hdlg, ID_EDIT_FIN,   NULL,  TRUE), track_e[7]);
    newdata->fo = (short)std::min((int)GetDlgItemInt(hdlg, ID_EDIT_FOUT,  NULL,  TRUE), track_e[8]);
    newdata->x  =               (short)GetDlgItemInt(hdlg, ID_EDIT_X,     NULL, FALSE);
    newdata->y  =               (short)GetDlgItemInt(hdlg, ID_EDIT_Y,     NULL, FALSE);

    newdata->auto_fade = FALSE != SendDlgItemMessage(hdlg, ID_EDIT_AUTO_FADE, BM_GETCHECK, 0, 0);
    newdata->auto_nr   = FALSE != SendDlgItemMessage(hdlg, ID_EDIT_AUTO_NR,   BM_GETCHECK, 0, 0);
